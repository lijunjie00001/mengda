...
import {
    StyleSheet,
    Text,
    View, Image, ActivityIndicator, Alert, DeviceEventEmitter, Platform
} from 'react-native';
...
export default class MessageText extends Component{
    ...
    componentWillMount() {
        ...
        if(Platform.OS=='android'){
            this.uploadAndoird=DeviceEventEmitter.addListener('uploadAndroid',(obj)=>{
                if(obj.id===this.props.Message.fromUser._id){
                    this.setState({
                        iconPath:'file://'+obj.iconPath
                    })
                }
            });
        }
    ...
    render(){
        console.log('............this.props.Message.text=====',this.props.Message.text)
        return(
            <View style={[{flexDirection:'row',paddingTop:15,},this.props.Message.fromUser._id==2?{paddingLeft:12,paddingRight:100,justifyContent:'flex-start'}:{paddingRight:12,paddingLeft:100,justifyContent:'flex-end'}]}>
                ...
                <View style={[{padding:13,borderRadius:5},this.props.Message.fromUser._id==2?{marginLeft:10,backgroundColor:colors.WHITE}:{backgroundColor:colors.CHATBUDDLE}]}>
                    <ASText style={{fontSize:16,color:colors.CHATTEXT}} text={this.props.Message.text}></ASText>
                </View>
