...
@containers()
export default class ChangeEmailsPage extends Component{
    static propTypes = {
    ...
    constructor(props){
        super(props)
        this.state={
            name:this.props.userInfo.email?this.props.userInfo.email:'',
    ...
    onPressSave(){
        if(!this.state.name){
            Alert.alert('提示','邮箱不能为空',[{text:'确定',style:'cancel'}]);
        }
        this.props.fetchData(this, '', Url.changeEmail(this.state.uid,this.state.name), {}, successCallback = (data) => {
            console.log('............名字',data)
            Alert.alert('提示','邮箱修改成功',[{text:'确定',style:'cancel',onPress:()=>{
                    let userinfo={
                        ...this.props.userInfo,
